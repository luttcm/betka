...
4. ✅ Реализовать auth + RBAC + подтверждение email при регистрации
...
7. ⏳ Реализовать settlement и проводки кошелька
...
## 15. План следующей итерации (bets-flow)

### 15.1 Цель итерации

Закрыть ядро этапа C по ставкам: от создания ставки до фиксации холда и подготовки к settlement.

### 15.2 Обязательные задачи

1. Ввести wallet-модуль в backend:
   - `GET /v1/wallet`;
   - `GET /v1/wallet/transactions`;
   - in-memory реализация на MVP с интерфейсом под будущий PostgreSQL repository.
2. Ввести bets-модуль:
   - `POST /v1/bets`;
   - `GET /v1/bets/my`;
   - валидация статуса события (только approved/open для приёма ставки).
3. Реализовать идемпотентность по `Idempotency-Key`:
   - требовать заголовок на `POST /v1/bets`;
   - правило уникальности `(user_id, idempotency_key)`;
   - повторный запрос с тем же ключом возвращает ранее созданную ставку.
4. Реализовать холд средств:
   - атомарная проверка доступного баланса;
   - запись `wallet_transactions` типа `hold`;
   - уменьшение доступного баланса на сумму ставки.
5. Расширить тестовое покрытие:
   - unit: валидация ставок и ledger-операции;
   - integration: успешный hold, отказ при недостаточном балансе, повтор с тем же `Idempotency-Key`.

### 15.3 Критерии готовности итерации

- Пользователь может поставить на опубликованное событие с валидным исходом.
- При недостаточном балансе API возвращает контролируемую ошибку 4xx.
- Повторный `POST /v1/bets` с тем же `Idempotency-Key` не создаёт дубликат.
- История транзакций кошелька отражает hold-операции.
- Тесты backend проходят командой `go test ./...`.

## 16. Старт этапа D (frontend MVP) — текущая итерация

### 16.1 Цель итерации

Запустить frontend-контур MVP и закрыть базовый пользовательский сценарий просмотра/создания событий.

### 16.2 Зафиксированный стек
...
### 16.3 Обязательные задачи

1. Инициализировать проект в `frontend/` и базовый layout.
2. Добавить API-клиент к backend `/v1` с конфигурацией `NEXT_PUBLIC_API_BASE_URL`.
3. Реализовать страницы MVP-итерации:
   - каталог событий;
   - карточка события;
   - создание события.
4. Добавить базовую обработку состояний запросов (loading/error/empty).
5. Подготовить инструкцию запуска frontend и краткое описание структуры.

### 16.4 Критерии готовности итерации

- Фронтенд запускается локально и обращается к backend API через конфиг.
- Пользователь может просмотреть список событий и открыть карточку события.
- Пользователь может создать событие через форму.
- Документация по запуску frontend обновлена.

### 16.5 Прогресс по итерации (2026-04-09)

- ✅ Инициализирован каркас frontend в `frontend/` (Next.js App Router + TypeScript + Tailwind конфиги).
- ✅ Добавлен API-слой под backend `/v1` с переменной `NEXT_PUBLIC_API_BASE_URL`.
- ✅ Реализованы страницы:
  - `/` — каталог событий;
  - `/events/[id]` — карточка события;
  - `/events/new` — форма создания события.
- ✅ Обновлены `frontend/README.md` и `.env.example`.
- ⚠️ Локальная проверка запуска не выполнена в текущем окружении: отсутствуют `node`/`npm`.
